...
// Команда для сброса мониторинга (для тестирования)
bot.command('reset_monitoring', async (ctx) => {
    try {
        const stats = await loadStats();
        stats.lastEpisodeId = null; // Сбрасываем, чтобы следующий эпизод считался новым
        await saveStats(stats);
        
        await ctx.reply('✅ Мониторинг сброшен. Следующая проверка RSS будет считать текущий эпизод новым.');
        
        // Тестируем автоматический мониторинг
        await ctx.reply('🔄 Тестирую автоматический мониторинг...');
        await checkForNewEpisodes(); // Запускаем без контекста = автоматический режим
        
        const chats = await loadChats();
        await ctx.reply(`📊 Результат: ${chats.subscribedChats.length} подписанных чатов для уведомлений`);
...
bot.command('update_menu', async (ctx) => {
    try {
        ...
        await ctx.reply('✅ Меню команд обновлено! Попробуйте ввести "/" или перезапустить чат.');
    } catch (error) {
        await ctx.reply(`❌ Ошибка обновления меню: ${error.message}`);
    }
});
...
bot.action('cmd_help', async (ctx) => {
    await ctx.answerCbQuery();
    ctx.reply(
        'Доступные команды (в алфавитном порядке):\n\n' +
        '🌍 /all - ВСЕ доступные рецензии (76 стран × 3 страницы)\n' +
        '🔍 /check_rss - диагностика RSS-мониторинга\n' +
        '❓ /help - показать эту справку\n' +
        '🗓️ /month - все рецензии за последний месяц\n' +
...
bot.launch().then(async () => {
    ...
    // Настраиваем меню команд (в алфавитном порядке)
    await bot.telegram.setMyCommands([
        { command: 'all', description: '🌍 ВСЕ доступные рецензии (может быть много!)' },
        { command: 'check_rss', description: '🔍 Диагностика RSS-мониторинга' },
        { command: 'help', description: '❓ Справка по командам' },
